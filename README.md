...
1. Build succeeds with both SPM packages resolved
2. App launches showing blog post list
3. Blog post cards render with correct styling (card shadow, rounded corners, chevron)
4. CTA banner renders with hero image, heading, body, button
5. CTA personalization responds to overrides set in the preview panel
6. Tapping a post navigates to detail with rich text body
7. Pull-to-refresh reloads content
8. Preview panel FAB opens the debug panel
9. Visual parity with the React Native reference app
10. Screen / view / click tracking fires (check console)
